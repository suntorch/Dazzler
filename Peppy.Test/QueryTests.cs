...
namespace Peppy.Test
{
   ...
   public class QueryTests<TProvider> : TestBase<TProvider> where TProvider : DatabaseProvider
   {
      ...
      public void SelectQuery_Basic()
      {
         ...
         var result = connection.Query<TestRecord>(CommandType.Text, "select 'hello peppy!'", ri: ri);
      ...
      string create_proc1 = @"
CREATE OR ALTER PROCEDURE PeppyProc1
	@Name varchar(50),
	@Age int
AS
BEGIN
...
END";
      ...
      public void SelectQuery_SP()
      {
         ...
         var result = connection.Query<TestRecord>(CommandType.StoredProcedure, "PeppyProc1", args);
         Assert.AreEqual(1, result.Count, "Invalid output record count.");
      ...
      string create_proc2 = @"
CREATE OR ALTER PROCEDURE PeppyProc2
AS BEGIN
...
END";
      ...
      public void SelectQuery_SP_with_RaiseError()
      {
         ...
         var result = connection.Query<TestRecord>(CommandType.StoredProcedure, "PeppyProc2", null);
         Assert.AreEqual(2, result.Count, "Invalid output record count.");
      }
